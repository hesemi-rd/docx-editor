...
Controlled comments (`comments` + `onCommentsChange`) are React-only today; the Vue adapter exposes the callback events but not the `comments` prop (see [Vue props](/docs/1.x/vue/props#comments-and-collaboration)).
...
The Vue adapter exposes the same areas as named slots (`#title-bar-left`, `#title-bar-right`, `#toolbar-extra`); see [Vue props](/docs/1.x/vue/props#title-bar-slots).
...
`agentPanel` is React-only; the Vue adapter pairs `useAgentBridge` with your own chat UI instead. Full setup in [Agents → Live editor](/docs/1.x/agents/live-editor).
